...
{
    public partial class StudentLog : DevExpress.XtraEditors.XtraForm
    {
        private string con;
        ...
        private void Fill()
        {
            ...
            try
            {
                ...
                if (count == "1")
                {
                    cmd.CommandText = "select `name`, `username`, `time`, `remarks` from `student_log`";
                    cmd.Connection = connected;
                    connected.Open();
                    ...
                    MessageBox.Show("Student Log is empty.");
            }
            catch (Exception err)
            {
        ...
        private void Previous_Click(object sender, EventArgs e)
        {
            try
            {
                if (empty == "No")
                {
                    logger = "Deleted the records of student log.";
                    Log();
                    connected.Open();
                    cmd.CommandText = "truncate table `student_log`";
                    cmd.Connection = connected;
                    cmd.ExecuteNonQuery();
                    connected.Close();
                    Fill();
                    ...
                    MessageBox.Show("Student Log is already empty");
            }
        ...
        private void Home_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                SI.Need(username, name, permission, schoolyear);
                SI.Show();
                this.Hide();
            }
        }

        private void StudentLog_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
                Home_Click(sender, EventArgs.Empty);

            else if (e.KeyCode == Keys.F1)
                MessageBox.Show("Alt + E = Export to Excel\nEscape = Back to Administrator Form" +
        ...
        private void StudentLog_FormClosed(object sender, FormClosedEventArgs e)
        {
